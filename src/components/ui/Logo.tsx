...
export function Logo({ color = "black", className, alt = "here*" }: LogoProps) {
  return (
    <img
      src="/logo.svg"
      alt={alt}
      width={56}
      height={60}
      className={`${color === "white" ? "invert" : ""} ${className ?? ""}`}
    />
  );
}

/** Full wordmark logo (58.5×20px) */
export function FullLogo({ color = "black", className, alt = "here*" }: LogoProps) {
  return (
    <img
      src="/logo-full.svg"
      alt={alt}
